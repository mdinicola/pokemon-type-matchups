...
def get_matchups_list() -> list[TypeMatchup]:
...
def get_defense_matchups(defense_types: list[str]) -> list[GroupMatchup]:
    matchups: list[GroupMatchup] = list(map(lambda x: filter(lambda y: y.defense_type == x, matchups_list), defense_types))
    
    if (len(defense_types) == 1):
        group_matchups: list[GroupMatchup] = list(map(lambda x: GroupMatchup(offense_type = x.offense_type, defense_types = defense_types, 
            effectiveness=x.effectiveness), matchups[0]))
